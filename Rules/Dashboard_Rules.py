...
def read_json_api(input_json):
    for val in input_json['Variables']:
            variables.append(val)
    for val in input_json['Values']:
            rule.append(val)
    return variables,rule


def run_api(input_json):
    variables,rule=read_json_api(input_json)
    df = pd.read_csv("Rules/all_data_dashboard-trial.csv")
    data = extract(df)
